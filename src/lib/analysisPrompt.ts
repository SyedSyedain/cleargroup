...
export function buildAnalysisPrompt(chat: string, participants: string[]): string {
  return `You are an expert project manager AI analyzing a WhatsApp group chat for an Indian college student project.
...
Be generous in extraction — it is better to include more than to miss something important.
...
1. tasks: Find EVERY instance where someone committed to doing something. Include even small tasks.
...
2. decisions: Find EVERY agreement, confirmation, technology choice, approach selection.
...
5. openQuestions: Find EVERY question that was asked but never clearly answered.
...
7. summary:
   ...
   - keyInsight:
     Write a specific, useful one-sentence insight about THIS project based on the actual content.
     NOT generic. Use real names and real issues.
     Example: "Vivek has gone silent for 2 days while backend remains incomplete before Friday deadline"
   - collaborationScore:
     Calculate based on:
   ...
   - mostActiveParticipant: The name with the most messages. Required field.
   - leastActiveParticipant: The name with fewest messages. Required field.
