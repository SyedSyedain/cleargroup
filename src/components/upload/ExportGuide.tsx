...
function PhoneFrame({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ width: 70, height: 116, background: "#1A2E3A", borderRadius: 8, border: "1.5px solid #2A4A5E", padding: "5px 4px", flexShrink: 0 }}>
      <div style={{ width: 16, height: 3, borderRadius: 99, background: "#0C1419", margin: "0 auto 4px" }} />
      <div style={{ height: "calc(100% - 7px)", background: "#0C1419", borderRadius: 4, overflow: "hidden" }}>{children}</div>
    </div>
  );
}
...
function Screen1() {
  return (
    <div style={{ padding: 5 }}>
      ...
      {[55, 70, 45].map((w, i) => (
        <div key={i} style={{ height: 4, width: `${w}%`, borderRadius: 99, marginBottom: 3, background: i % 2 === 0 ? "#1A223A" : "#1A2E3A", marginLeft: i % 2 === 0 ? "auto" : 0 }} />
      ))}
    </div>
  );
}
    ...
    <div style={{ padding: 5 }}>
      ...
      {[0, 1].map((i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 3, padding: "3px 2px", borderRadius: 3, marginBottom: 3, background: i === 1 ? "rgba(99,102,241,0.12)" : "transparent", border: i === 1 ? "1px solid rgba(99,102,241,0.3)" : "none" }}>
          <div style={{ width: 5, height: 5, borderRadius: "50%", flexShrink: 0, background: i === 1 ? "#6366F1" : "#2A4A5E" }} />
          <div style={{ flex: 1, height: 3, borderRadius: 99, background: i === 1 ? "rgba(99,102,241,0.4)" : "#1A2E3A" }} />
        </div>
      ))}
    </div>
  );
...
export default function ExportGuide({ onClose }: { onClose: () => void }) {
  return (
    <div style={{ marginTop: 8, padding: "12px 14px", borderRadius: 12, background: "#111E26", border: "1px solid #1A2E3A" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: "white" }}>How to export from WhatsApp</p>
        <button onClick={onClose} title="Close" style={{ background: "none", border: "none", cursor: "pointer", padding: 2, borderRadius: 4, color: "#8899AA", display: "flex" }}>
          <X size={13} />
        </button>
      </div>
