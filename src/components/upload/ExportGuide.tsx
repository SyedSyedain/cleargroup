    ...
    <div style={{ padding: 5 }}>
      ...
      {[55, 70, 45].map((w, i) => (
        <div key={i} style={{ height: 4, width: `${w}%`, borderRadius: 99, marginBottom: 3,
          background: i % 2 === 0 ? "#1A223A" : "#1A2E3A", marginLeft: i % 2 === 0 ? "auto" : 0 }} />
      ))}
    </div>
  );
}
    ...
    <div style={{ padding: 5 }}>
      ...
      {[0, 1].map((i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 3, padding: "3px 2px",
          ...
          border: i === 1 ? "1px solid rgba(99,102,241,0.3)" : "none" }}>
          <div style={{ width: 5, height: 5, borderRadius: "50%", flexShrink: 0,
            background: i === 1 ? "#6366F1" : "#2A4A5E" }} />
          <div style={{ flex: 1, height: 3, borderRadius: 99,
            background: i === 1 ? "rgba(99,102,241,0.4)" : "#1A2E3A" }} />
        </div>
      ))}
    </div>
  );
...
const STEPS = [
  ...
  { num: 2, title: "Tap Export Chat",  sub: "Group name â†’ Export Chat â†’ Without Media" },
  { num: 3, title: "Share the file",    sub: "A .txt or .zip file will be saved. Both formats work â€” upload either one" },
];
        ...
        <div key={num} style={{ display: "flex", gap: 10, alignItems: "flex-start",
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center",
            ...
            <div style={{ width: 20, height: 20, borderRadius: "50%", display: "flex",
              ...
              border: "1.5px solid #6366F1" }}>{num}</div>
