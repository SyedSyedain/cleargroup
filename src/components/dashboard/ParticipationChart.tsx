...
function pickChampion(people: ParticipationStat[]): ParticipationStat | null {
  ...
  return [...people].sort(
    (a, b) => b.tasksCompleted - a.tasksCompleted || b.tasksAssigned - a.tasksAssigned || b.messageCount - a.messageCount
  )[0] ?? null;
}
...
export default function ParticipationChart({ analysis }: Props) {
  ...
  const mostActive =
    summary.mostActiveParticipant ||
    (safePeople.length > 0 ? safePeople.reduce((a, b) => (a.messageCount > b.messageCount ? a : b)).name : null);
  const leastActive =
    summary.leastActiveParticipant ||
    (safePeople.length > 1 ? safePeople.reduce((a, b) => (a.messageCount < b.messageCount ? a : b)).name : null);
  const collaborationScore =
    typeof summary.collaborationScore === "number" && summary.collaborationScore > 0
        ...
        {displayPeople.map((person, index) => (
          ...
          >
            <div className="w-full sm:w-[180px] sm:min-w-[180px] flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-full flex items-center justify-center font-semibold" style={{ background: "#6366F1", color: "#060810" }}>
      ...
      <div className="mt-8 grid grid-cols-1 xl:grid-cols-[1.4fr_1fr] gap-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 min-w-0">
          ...
          <div className="rounded-xl p-4 min-w-0" style={{ background: "#111828", border: "1px solid #1A2440" }}>
            ...
            <p className="text-white font-semibold mt-1 leading-snug break-words">{tasksChampion?.name ?? "No tasks completed yet"}</p>
          ...
          <div className="rounded-xl p-4 min-w-0" style={{ background: "#111828", border: "1px solid #1A2440" }}>
            ...
            <p className="text-white font-semibold mt-1 leading-snug break-words">{leastActive ?? "Balanced team"}</p>
            <p className="text-xs mt-1" style={{ color: "#7A92B8" }}>{leastActive ? `Only ${leastStats?.messageCount ?? 0} messages` : "Everyone participated equally"}</p>
          </div>
        </div>
        <div className="rounded-xl p-5 flex items-center justify-center" style={{ background: "#111828", border: "1px solid #1A2440" }}>
