...
function Counter({ value }: { value: number }) {
  return <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}>{value}%</motion.span>;
}
    ...
    <motion.section
      ...
      <div className="space-y-4">
        {people.map((person, index) => (
          <motion.div key={person.name} className="flex items-center gap-4" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: index * 0.08 }}>
            <div className="w-[180px] min-w-[180px] flex items-center gap-3">
            ...
            <div className="flex-1 h-2 rounded" style={{ background: "#1A2E3A" }}>
            ...
            <div className="w-[60px] text-right text-white font-semibold"><Counter value={Math.round(person.participationPercentage)} /></div>
          </motion.div>
        ))}
      </div>
