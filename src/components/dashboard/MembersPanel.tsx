...
export default function MembersPanel({ projectId, ownerId }: Props) {
  ...
  return (
    <div className="rounded-xl p-5" style={{ background: "#0C1121", border: "1px solid #1A2440" }}>
      ...
      {sortedMembers.length === 0 ? (
        <div className="rounded-lg p-4 text-sm" style={{ background: "#111828", color: "#7A92B8", border: "1px dashed #1A2440" }}>
        ...
        <div className="space-y-3">
          {sortedMembers.map((member, index) => {
            ...
            return (
              <motion.div key={member.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.25, delay: index * 0.05 }} className="rounded-lg px-3 py-2.5" style={{ background: "#111828", border: "1px solid #1A2440" }}>
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center font-semibold" style={{ background: "#6366F1", color: "#060810", fontSize: 12 }}>{member.user_name.slice(0, 1).toUpperCase()}</div>
                    <div className="min-w-0">
                      ...
                      <p className="text-xs truncate" style={{ color: "#7A92B8" }}>{member.user_email}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="flex items-center justify-end gap-1"><span className="inline-block w-2 h-2 rounded-full" style={{ background: "#8B5CF6" }} /><span className="text-[10px]" style={{ color: "#7A92B8" }}>Online</span></div>
                    <p className="text-[10px]" style={{ color: "#7A92B8" }}>{joinedAgo(member.joined_at)}</p>
                  </div>
                </div>
              </motion.div>
            );
