  ...
  return (
    <div style={{ minWidth: 240, flexShrink: 0, opacity: dimmed ? 0.85 : 1 }}>
      <div className="flex items-center gap-3 mb-3">
        <div className="flex items-center justify-center rounded-full shrink-0 font-bold text-white" style={{ width: 40, height: 40, background: "#0ABFBC", fontSize: 16 }}>{initial}</div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {highlighted && <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ background: "#0ABFBC20", color: "#0ABFBC", border: "1px solid #0ABFBC40" }}>Your tasks</span>}
          </div>
          <div className="rounded-full overflow-hidden" style={{ height: 3, background: "#1A2E3A" }}><div style={{ width: `${pct}%`, height: "100%", background: "#0ABFBC", borderRadius: 9999, transition: "width 0.6s ease" }} /></div>
        </div>
      </div>

      <div style={{ background: "#0C1419", border: "1px solid #1A2E3A", borderRadius: 12, padding: 16, minHeight: 200, boxShadow: highlighted ? "0 0 0 1px #0ABFBC inset, 0 12px 24px rgba(10,191,188,0.15)" : "none" }}>
        <AnimatePresence mode="popLayout">
          {tasks.length > 0
            ...
            : <div className="flex flex-col items-center justify-center gap-2 py-8"><Check size={20} style={{ color: "#0ABFBC" }} /><p className="text-xs text-center" style={{ color: "#3A5060" }}>No tasks match filter</p></div>}
        </AnimatePresence>
        ...
        {adding ? (
          <div className="mt-1">
            ...
            <div className="flex gap-2">
              <button onClick={handleAdd} className="flex-1 text-xs font-semibold rounded-lg py-1.5" style={{ background: "rgba(10,191,188,0.15)", color: "#0ABFBC", border: "1px solid rgba(10,191,188,0.3)", cursor: "pointer" }}>Add</button>
