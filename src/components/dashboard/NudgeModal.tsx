...
function template(name: string, taskText: string, deadline: string | null) {
  const line = deadline
    ...
    : "No strict deadline yet, but a quick update would help.";
...
export default function NudgeModal({ isOpen, personName, taskText, deadline, onClose }: Props) {
  ...
  useEffect(() => {
    if (isOpen) {
      setMessage(template(personName, taskText, deadline));
      setCopied(false);
    }
  }, [isOpen, personName, taskText, deadline]);
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <button onClick={onClose} className="absolute right-4 top-4" style={{ color: "#7A92B8" }}>
              <X size={18} />
            </button>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-white font-semibold text-lg">Message for {personName}</h3>
            </div>
