...
function TypingDots() {
  return (
    <div className="flex gap-1 px-4 py-3 rounded-[14px]" style={{ background: "#111828" }}>
      {[0, 1, 2].map((d) => (
        <motion.span key={d} className="h-2 w-2 rounded-full" style={{ background: "#7A92B8" }}
          animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: d * 0.12 }} />
      ))}
    </div>
  );
}
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} viewport={{ once: true }}>
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-white font-semibold text-xl">Ask AI</h2>
      </div>
      ...
      <div className="rounded-[14px] border h-[500px] flex flex-col" style={{ background: "#0C1121", borderColor: "#1A2440" }}>
        <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: "#1A2440" }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "#6366F1" }}>
              <Bot size={16} style={{ color: "#060810" }} />
            </div>
            <div>
              <p className="text-sm text-white font-medium">ClearGroup AI</p>
              <p className="text-xs" style={{ color: "#7A92B8" }}>Powered by Gemini</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {SUGGESTIONS.map((s) => (
                <button key={s} onClick={() => void sendMessage(s)}
                  className="text-[13px] px-4 py-2 rounded-full border transition-colors hover:border-[#2A3860]"
                  style={{ background: "#111828", borderColor: "#1A2440", color: "#E8F4F8" }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              {m.role === "assistant" && (
                <div className="w-7 h-7 rounded-full flex items-center justify-center mr-2 mt-1" style={{ background: "#111828" }}>
                  <Bot size={14} style={{ color: "#6366F1" }} />
                </div>
              )}
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                className="px-3.5 py-2.5 max-w-[85%] text-sm leading-relaxed"
                style={{
                  background: m.role === "user" ? "#6366F1" : (m.isError ? "#1A0A0A" : "#111828"),
                  color: m.role === "user" ? "#060810" : (m.isError ? "#FF6B6B" : "#E8F4F8"),
                  borderRadius: m.role === "user" ? "14px 14px 4px 14px" : "14px 14px 14px 4px",
                  maxWidth: m.role === "user" ? "75%" : "85%",
                  border: m.isError ? "1px solid #FF3333" : "none",
                }}>
                {m.content}
              </motion.div>
            </div>
