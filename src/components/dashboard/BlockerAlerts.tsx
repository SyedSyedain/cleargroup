...
const TYPE_LABEL: Record<BType, string> = {
  silent_member: "Silent Member", unresolved_conflict: "Unresolved Conflict",
  missing_response: "No Response", unclear_ownership: "Unclear Ownership",
};
...
function BlockerCard({ b, i }: { b: Blocker; i: number }) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        borderTop: `1px solid ${color}28`, borderRight: `1px solid ${color}28`,
        borderBottom: `1px solid ${color}28`, borderLeft: `4px solid ${color}`,
      ...
      {b.involvedPerson && (
        <div className="flex items-center gap-2 mb-2">
          <div className="flex items-center justify-center rounded-full font-bold text-white shrink-0"
            style={{ width: 28, height: 28, background: color, fontSize: 12 }}>{initial}</div>
          <span className="text-white font-semibold text-sm">{b.involvedPerson}</span>
        </div>
      )}
      ...
      <button className="mt-3 text-xs font-semibold"
        style={{ background: "none", border: "none", cursor: "pointer", color: "#0ABFBC" }}>
...
export default function BlockerAlerts({ blockers }: { blockers: Blocker[] }) {
  return (
    <div>
      <div className="flex items-center gap-2.5 mb-4">
        <AlertTriangle size={20} style={{ color: "#FFB347" }} />
        <h2 className="font-semibold text-white" style={{ fontSize: 20 }}>Blockers & Risks</h2>
      </div>
      ...
      {blockers.length === 0 ? (
        <div className="flex items-center gap-3 p-5 rounded-xl"
          ...
          <p className="text-sm font-medium" style={{ color: "#06D6A0" }}>
            No blockers detected! Your project is on track.
          </p>
        </div>
      ) : (
        <div>{blockers.map((b, i) => <BlockerCard key={b.id} b={b} i={i} />)}</div>
      )}
    </div>
  );
}
