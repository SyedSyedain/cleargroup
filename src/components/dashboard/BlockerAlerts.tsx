...
export default function BlockerAlerts({ blockers, onGenerateNudge }: BlockerAlertsProps) {
  return (
    <div>
      ...
      {blockers.length === 0 ? <div className="flex items-center gap-3 p-5 rounded-xl" style={{ background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.25)" }}><span style={{ fontSize: 22 }}>OK</span><p className="text-sm font-medium" style={{ color: "#8B5CF6" }}>No blockers detected! Your project is on track.</p></div> : <div>{blockers.map((b, i) => <BlockerCard key={b.id} b={b} i={i} onGenerateNudge={onGenerateNudge} />)}</div>}
    </div>
  );
}
